...
export class Predicate<T> {
  ...
  public static of = <T>(condition: PredicateType<T>) =>
    new Predicate(condition);

  private static isInstance = <T>(
    input: Predicate<T> | PredicateType<T>,
  ): Predicate<T> => (input instanceof Predicate ? input : Predicate.of(input));
